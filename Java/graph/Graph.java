...
public class Graph {
		...
		while (!queue.isEmpty()) {
			...
			for (String nbr : nbrs) {
				...
				if (!processed.containsKey(nbr)) {
					// make a new pair and put it in queue
	...
	private class PrimsPair implements Comparable<PrimsPair> {
		...
		public PrimsPair(String vname, String acqname, int cost) {
			this.vname = vname;
			this.acqvname = acqname;
			this.cost = cost;
		}
	...
	public Graph prims() {
		...
		for (String key : vtces.keySet()) {
			...
			PrimsPair np = new PrimsPair(key, null, Integer.MAX_VALUE);
			heap.add(np);
			map.put(key, np);

		}

		// till heap is not empty, keep on removing pairs
		while (!heap.isEmpty()) {
	...
	public HashMap<String, Integer> dijkstra(String src) {
		...
		for (String key : vtces.keySet()) {
			...
			DijkstraPair np = new DijkstraPair(key, "", Integer.MAX_VALUE);
